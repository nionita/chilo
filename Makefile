...
NNUE_EVAL_BENCH_SRC := nnue_eval_bench.cpp
...
RELEASE_AVX2_BINS := $(RELEASE_AVX2_DIR)/perft $(RELEASE_AVX2_DIR)/perft_diag $(RELEASE_AVX2_DIR)/engine_tests $(RELEASE_AVX2_DIR)/chilo $(RELEASE_AVX2_DIR)/selfplay_collect $(RELEASE_AVX2_DIR)/eval_fen $(RELEASE_AVX2_DIR)/nnue_eval_bench $(RELEASE_AVX2_DIR)/futility_stats $(RELEASE_AVX2_DIR)/futility_probe $(RELEASE_AVX2_DIR)/move_ordering_probe
DEBUG_BINS := $(DEBUG_DIR)/perft_debug $(DEBUG_DIR)/perft_diag_debug $(DEBUG_DIR)/engine_tests_debug $(DEBUG_DIR)/chilo_debug $(DEBUG_DIR)/selfplay_collect_debug $(DEBUG_DIR)/eval_fen_debug $(DEBUG_DIR)/futility_stats_debug $(DEBUG_DIR)/futility_probe_debug $(DEBUG_DIR)/move_ordering_probe_debug
VALIDATE_BINS := $(VALIDATE_DIR)/perft_validate $(VALIDATE_DIR)/perft_diag_validate $(VALIDATE_DIR)/engine_tests_validate $(VALIDATE_DIR)/chilo_validate $(VALIDATE_DIR)/selfplay_collect_validate $(VALIDATE_DIR)/eval_fen_validate $(VALIDATE_DIR)/futility_stats_validate $(VALIDATE_DIR)/futility_probe_validate $(VALIDATE_DIR)/move_ordering_probe_validate
WIN64_BINS := $(WIN64_DIR)/perft.exe $(WIN64_DIR)/perft_diag.exe $(WIN64_DIR)/engine_tests.exe $(WIN64_DIR)/chilo.exe $(WIN64_DIR)/selfplay_collect.exe $(WIN64_DIR)/eval_fen.exe $(WIN64_DIR)/nnue_eval_bench.exe $(WIN64_DIR)/futility_stats.exe $(WIN64_DIR)/futility_probe.exe $(WIN64_DIR)/move_ordering_probe.exe
WIN64_AVX2_BINS := $(WIN64_AVX2_DIR)/perft.exe $(WIN64_AVX2_DIR)/perft_diag.exe $(WIN64_AVX2_DIR)/engine_tests.exe $(WIN64_AVX2_DIR)/chilo.exe $(WIN64_AVX2_DIR)/selfplay_collect.exe $(WIN64_AVX2_DIR)/eval_fen.exe $(WIN64_AVX2_DIR)/nnue_eval_bench.exe $(WIN64_AVX2_DIR)/futility_stats.exe $(WIN64_AVX2_DIR)/futility_probe.exe $(WIN64_AVX2_DIR)/move_ordering_probe.exe
...
nnue_eval_bench: $(RELEASE_DIR)/nnue_eval_bench
...
futility_probe_debug: $(DEBUG_DIR)/futility_probe_debug
move_ordering_probe_debug: $(DEBUG_DIR)/move_ordering_probe_debug
...
futility_probe_validate: $(VALIDATE_DIR)/futility_probe_validate
move_ordering_probe_validate: $(VALIDATE_DIR)/move_ordering_probe_validate
...
nnue_eval_bench.exe: $(WIN64_DIR)/nnue_eval_bench.exe
...
$(RELEASE_DIR)/nnue_eval_bench: $(NNUE_EVAL_BENCH_SRC) $(ENGINE_OBJ) | $(RELEASE_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) -O3 -DNDEBUG -o $@ $(NNUE_EVAL_BENCH_SRC) $(ENGINE_OBJ)
...
$(RELEASE_DIR)/futility_probe: $(FUTILITY_PROBE_SRC) $(ENGINE_OBJ) | $(RELEASE_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) -O3 -DNDEBUG -o $@ $(FUTILITY_PROBE_SRC) $(ENGINE_OBJ)
...
$(RELEASE_DIR)/futility_margin_tail_tests: $(FUTILITY_MARGIN_TAIL_TEST_SRC) $(FUTILITY_MARGIN_TAIL_SRC) chess_position.h | $(RELEASE_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) -O3 -DNDEBUG -o $@ $(FUTILITY_MARGIN_TAIL_TEST_SRC)
...
$(RELEASE_AVX2_DIR)/nnue_eval_bench: $(NNUE_EVAL_BENCH_SRC) $(ENGINE_AVX2_OBJ) | $(RELEASE_AVX2_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) $(AVX2_CPPFLAGS) -O3 -DNDEBUG -o $@ $(NNUE_EVAL_BENCH_SRC) $(ENGINE_AVX2_OBJ)
...
$(RELEASE_AVX2_DIR)/futility_probe: $(FUTILITY_PROBE_SRC) $(ENGINE_AVX2_OBJ) | $(RELEASE_AVX2_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) $(AVX2_CPPFLAGS) -O3 -DNDEBUG -o $@ $(FUTILITY_PROBE_SRC) $(ENGINE_AVX2_OBJ)
...
$(DEBUG_DIR)/futility_probe_debug: $(FUTILITY_PROBE_SRC) $(ENGINE_DEBUG_OBJ) | $(DEBUG_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) -O0 -g -o $@ $(FUTILITY_PROBE_SRC) $(ENGINE_DEBUG_OBJ)

$(DEBUG_DIR)/move_ordering_probe_debug: $(MOVE_ORDERING_PROBE_SRC) $(ENGINE_DEBUG_OBJ) | $(DEBUG_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) -O0 -g -o $@ $(MOVE_ORDERING_PROBE_SRC) $(ENGINE_DEBUG_OBJ)
...
$(VALIDATE_DIR)/futility_probe_validate: $(FUTILITY_PROBE_SRC) $(ENGINE_VALIDATE_OBJ) | $(VALIDATE_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) -O0 -g -DCHESS_VALIDATE_STATE -o $@ $(FUTILITY_PROBE_SRC) $(ENGINE_VALIDATE_OBJ)

$(VALIDATE_DIR)/move_ordering_probe_validate: $(MOVE_ORDERING_PROBE_SRC) $(ENGINE_VALIDATE_OBJ) | $(VALIDATE_DIR)
	$(CXX) $(CXXFLAGS) $(EXTRA_CPPFLAGS) -O0 -g -DCHESS_VALIDATE_STATE -o $@ $(MOVE_ORDERING_PROBE_SRC) $(ENGINE_VALIDATE_OBJ)
...
$(WIN64_DIR)/nnue_eval_bench.exe: $(NNUE_EVAL_BENCH_SRC) $(ENGINE_WIN64_OBJ) | $(WIN64_DIR)
	$(WIN64_CXX) $(WIN64_CXXFLAGS) $(EXTRA_CPPFLAGS) -O3 -DNDEBUG $(WIN64_LDFLAGS) -o $@ $(NNUE_EVAL_BENCH_SRC) $(ENGINE_WIN64_OBJ)
...
$(WIN64_DIR)/futility_probe.exe: $(FUTILITY_PROBE_SRC) $(ENGINE_WIN64_OBJ) | $(WIN64_DIR)
	$(WIN64_CXX) $(WIN64_CXXFLAGS) $(EXTRA_CPPFLAGS) -O3 -DNDEBUG $(WIN64_LDFLAGS) -o $@ $(FUTILITY_PROBE_SRC) $(ENGINE_WIN64_OBJ)
...
$(WIN64_AVX2_DIR)/nnue_eval_bench.exe: $(NNUE_EVAL_BENCH_SRC) $(ENGINE_WIN64_AVX2_OBJ) | $(WIN64_AVX2_DIR)
	$(WIN64_CXX) $(WIN64_CXXFLAGS) $(EXTRA_CPPFLAGS) $(AVX2_CPPFLAGS) -O3 -DNDEBUG $(WIN64_LDFLAGS) -o $@ $(NNUE_EVAL_BENCH_SRC) $(ENGINE_WIN64_AVX2_OBJ)
...
$(WIN64_AVX2_DIR)/futility_probe.exe: $(FUTILITY_PROBE_SRC) $(ENGINE_WIN64_AVX2_OBJ) | $(WIN64_AVX2_DIR)
	$(WIN64_CXX) $(WIN64_CXXFLAGS) $(EXTRA_CPPFLAGS) $(AVX2_CPPFLAGS) -O3 -DNDEBUG $(WIN64_LDFLAGS) -o $@ $(FUTILITY_PROBE_SRC) $(ENGINE_WIN64_AVX2_OBJ)
...
clean:
	rm -rf $(BUILD_DIR)
	rm -f perft perft_diag engine_tests chilo selfplay_collect eval_fen nnue_eval_bench futility_stats futility_probe move_ordering_probe perft_tests perft_debug perft_diag_debug engine_tests_debug chilo_debug selfplay_collect_debug eval_fen_debug futility_stats_debug futility_probe_debug move_ordering_probe_debug perft_tests_debug perft_validate perft_diag_validate engine_tests_validate chilo_validate selfplay_collect_validate eval_fen_validate futility_stats_validate futility_probe_validate move_ordering_probe_validate perft_tests_validate perft.exe perft_diag.exe engine_tests.exe chilo.exe selfplay_collect.exe eval_fen.exe nnue_eval_bench.exe futility_stats.exe futility_probe.exe move_ordering_probe.exe perft_tests.exe attack.o movegen.o make_unmake.o perft_lib.o eval.o search.o attack.debug.o movegen.debug.o make_unmake.debug.o perft_lib.debug.o eval.debug.o search.debug.o attack.validate.o movegen.validate.o make_unmake.validate.o perft_lib.validate.o eval.validate.o search.validate.o attack.win64.o movegen.win64.o make_unmake.win64.o perft_lib.win64.o eval.win64.o search.win64.o
