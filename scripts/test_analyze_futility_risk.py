...
class RiskAnalysisTest(unittest.TestCase):
    def test_tail_and_semantic_regressions_are_calculated_from_root_scores(self) -> None:
        ...
        metrics = analyze_futility_risk.compute_risk_metrics(
            reference, candidate, [first, second], 600.0,
            [0.5], [0.1, 0.5], 150, -150,
        )
        self.assertEqual(metrics["position_count"], 2)
        self.assertGreater(metrics["absolute_regret"]["tail_mean"]["top_0.5"], 0.0)
        self.assertGreater(metrics["absolute_regret"]["mean_squared"], 0.0)
        self.assertEqual(metrics["semantic_regressions_vs_reference"]["winning_mate_missed"], 1)
        self.assertEqual(metrics["semantic_regressions_vs_reference"]["clear_advantage_lost"], 1)
        self.assertEqual(metrics["semantic_regressions_vs_reference"]["clear_advantage_to_nonpositive"], 1)
        self.assertEqual(metrics["semantic_regressions_vs_reference"]["nonlosing_to_losing"], 1)
