...
- Search now uses per-ply NNUE accumulator frames. Each searched child starts from a copy of the parent accumulator and applies its move delta once, so returning from the child does not need NNUE undo work. `evaluate(pos)` remains the full-rebuild reference path for tools and tests, and `performance.md` contains the branch-local before/after profiling for this change.
