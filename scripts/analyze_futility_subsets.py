...
def run(config_path: Path, output_dir: Path) -> Dict[str, Any]:
    ...
    result = {"schema": SCHEMA, "config": {"path": str(config_path), "sha256": hashlib.sha256(config_path.read_bytes()).hexdigest()}, "anchor": {"reference": anchor.reference_identity, "baseline": anchor.baseline_identity, "trusted_set": anchor.trusted_set, "rescue": dict(anchor.rescue) if anchor.rescue is not None else None}, "variants": [{key: value for key, value in item.items() if key != "candidate"} for item in variants], "samples": results, "summary": summaries, "pairwise_vs_baseline": pairwise}
